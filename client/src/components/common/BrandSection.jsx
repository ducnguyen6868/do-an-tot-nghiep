      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid grid-cols-3 md:grid-cols-5 lg:grid-cols-10 gap-4">
          {brands.map((brand, index) => (
            <div
            key={brand._id}
              className={`group relative overflow-hidden rounded-xl transition-all duration-300 transform hover:scale-105 animate-fadeIn ${index === activeIndex
            ...
            >
              ...
              <div className={`absolute bottom-0 left-0 right-0 p-1 bg-white/95 
                backdrop-blur-sm transform translate-y-full group-hover:translate-y-0
                 transition-transform duration-300 ${index === activeIndex ?'translate-y-0':''}`}>
                <h4 className="font-semibold text-center lg:text-sm md:text-xs sm:text-xs text-gray-900">{brand.name}</h4>
              </div>
              {index === activeIndex && (
